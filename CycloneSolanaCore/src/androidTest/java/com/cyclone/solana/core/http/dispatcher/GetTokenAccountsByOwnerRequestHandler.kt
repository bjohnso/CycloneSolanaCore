...
object GetTokenAccountsByOwnerRequestHandler: MockRequest {
    ...
    override fun getErrorResponse(): MockRequestHandler {
        return { request ->
            when (validateRequest(request)) {
                true -> respond(
                    content = FileReader.instance.readJsonFile(
                        FileReader.FileResource.getTokenAccountsByOwnerError
                    ),
                    headers = headersOf("Content-Type" to listOf("application/json"))
                )
                else -> error("Request validation failed")
    ...
    override suspend fun validateRequest(request: HttpRequestData): Boolean {
        ...
        val requestModel = json.decodeFromString(
            RPCRequest.RequestGetTokenAccountsByOwner.serializer(),
            requestJson
        )

        return requestModel.id == RPC.RPCMethodIds.GET_TOKEN_ACCOUNTS_BY_OWNER &&
                requestModel.method == RPC.RPCMethods.GET_TOKEN_ACCOUNTS_BY_OWNER &&
                requestModel.params.size == 3
    }
}